...
angular.module('sysMarket.controllers', ['sysMarket.services'])
    ...
    .controller('ItemListFCtrl', ['$scope', '$route', '$routeParams', 'syscoinService', function($scope, $route, $routeParams, syscoinService) {
        updateNavClasses($route.current.$$route.originalPath);
    ...
    .controller('AdminCtrl', ['$scope', '$route', '$routeParams', '$http', 'syscoinService', function($scope, $route, $routeParams, $http, syscoinService) {
        updateNavClasses($route.current.$$route.originalPath);
        ...
        $scope.submitData = function (item, resultVarName)
        {
	        var config = {
		        params: {
		        	//id: id,
			    ...
			    console.log("submit func called: ", item);
			    /* $.ajax({
				    type: "POST",
                ...
                request.then(function(response){console.log("Ajax call success");});
		};
    }])
        ...
        $('#dividendBtn').click(function() {
            ...
            var request = syscoinService.getItem(title);
              request.then(function(response) { 
				...
				console.log("return data accepts 1 = ", data);
				...
				for(var i = 0; i < items.length; i++){
					...
					raw.then(function(response) { 
						...
						txdata.then(function(response) { 
							...
							console.log("decoderaw response: ",response.data);
...
function updateNavClasses(currentRoute) {
    ...
    $('#items-f-nav').removeClass("active");
